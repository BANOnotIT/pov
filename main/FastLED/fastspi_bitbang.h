...
private:
    ...
    template<uint8_t BIT>
    __attribute__((always_inline)) inline static void writeBit(uint8_t b, clock_ptr_t clockpin, data_ptr_t datapin) {
        if (b & (1 << BIT)) {
            FastPin<DATA_PIN>::hi(datapin);
            FastPin<CLOCK_PIN>::hi(clockpin);
            CLOCK_HI_DELAY;
            FastPin<CLOCK_PIN>::lo(clockpin);
            CLOCK_LO_DELAY;
        } else {
    ...
    template<uint8_t BIT>
    __attribute__((always_inline)) inline static void writeBit(uint8_t b, clock_ptr_t clockpin, data_ptr_t datapin,
                                                               data_t hival, data_t loval, clock_t hiclock,
                                                               clock_t loclock) {
    ...
    // combinations
    template<uint8_t BIT>
    __attribute__((always_inline)) inline static void writeBit(uint8_t b, data_ptr_t clockdatapin,
        ...
        if (b & (1 << BIT)) {
            ...
            FastPin<DATA_PIN>::fastset(clockdatapin, datahiclockhi);
            CLOCK_HI_DELAY;
            FastPin<DATA_PIN>::fastset(clockdatapin, datahiclocklo);
            CLOCK_LO_DELAY;
        } else {
            ...
            FastPin<DATA_PIN>::fastset(clockdatapin, dataloclockhi);
            CLOCK_HI_DELAY;
            FastPin<DATA_PIN>::fastset(clockdatapin, dataloclocklo);
            CLOCK_LO_DELAY;
        }
    ...
    __attribute__((noinline)) void writePixels(PixelController<RGB_ORDER> pixels) {
        ...
        if (FastPin<DATA_PIN>::port() != FastPin<CLOCK_PIN>::port()) {
            ...
            while (pixels.has(1)) {
                ...
                writeByte(D::adjust(pixels.loadAndScale0()), datapin, datahi_clockhi, datalo_clockhi, datahi_clocklo,
                          datalo_clocklo);
                writeByte(D::adjust(pixels.loadAndScale1()), datapin, datahi_clockhi, datalo_clockhi, datahi_clocklo,
                          datalo_clocklo);
                writeByte(D::adjust(pixels.loadAndScale2()), datapin, datahi_clockhi, datalo_clockhi, datahi_clocklo,
                          datalo_clocklo);
                pixels.advanceData();
                pixels.stepDithering();
            }
