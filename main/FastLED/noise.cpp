...
FL_PROGMEM static uint8_t const
p[] = {
...
int8_t inoise8_raw(uint16_t x, uint16_t y, uint16_t z) {
    ...
    u = scale8_LEAVING_R1_DIRTY(u, u);
    v = scale8_LEAVING_R1_DIRTY(v, v);
    w = scale8(w, w);
...
void fill_raw_2dnoise8(uint8_t *pData, int width, int height, uint8_t octaves, q44 freq44, fract8 amplitude, int skip,
                       uint16_t x, int scalex, uint16_t y, int scaley, uint16_t time) {
    if (octaves > 1) {
...
void
fill_raw_2dnoise16(uint16_t *pData, int width, int height, uint8_t octaves, q88 freq88, fract16 amplitude, int skip,
                   uint32_t x, int scalex, uint32_t y, int scaley, uint32_t time) {
    if (octaves > 1) {
...
void
fill_raw_2dnoise16into8(uint8_t *pData, int width, int height, uint8_t octaves, q44 freq44, fract8 amplitude, int skip,
                        uint32_t x, int scalex, uint32_t y, int scaley, uint32_t time) {
    if (octaves > 1) {
        fill_raw_2dnoise16into8(pData, width, height, octaves - 1, freq44, amplitude, skip + 1, x * freq44,
                                scalex * freq44, y * freq44, scaley * freq44, time);
    } else {
        // amplitude is always 255 on the lowest level
        amplitude = 255;
    }
...
void fill_2dnoise8(CRGB *leds, int width, int height, bool serpentine,
                   ...
                   uint8_t hue_octaves, uint16_t hue_x, int hue_xscale, uint16_t hue_y, uint16_t hue_yscale,
                   uint16_t hue_time, bool blend) {
    ...
    fill_raw_2dnoise8((uint8_t * )
    V, width, height, octaves, x, xscale, y, yscale, time);
    fill_raw_2dnoise8((uint8_t * )
    H, width, height, hue_octaves, hue_x, hue_xscale, hue_y, hue_yscale, hue_time);
...
void fill_2dnoise16(CRGB *leds, int width, int height, bool serpentine,
                    ...
                    uint8_t hue_octaves, uint16_t hue_x, int hue_xscale, uint16_t hue_y, uint16_t hue_yscale,
                    uint16_t hue_time, bool blend, uint16_t hue_shift) {
    uint8_t V[height][width];
    uint8_t H[height][width];
    ...
    fill_raw_2dnoise16into8((uint8_t * )
    V, width, height, octaves, q44(2, 0), 171, 1, x, xscale, y, yscale, time);
